...
object PersistentModel extends Model:
  ...
  def update(id: Id)(f: Task => Task): Option[Task] =
    ...
    val updatedTasks = tasks.map { (taskId, task) =>
      if taskId == id then
        taskId -> f(task)
      else
        taskId -> task
  ...
  def tasks(tag: Tag): Tasks =
    val tasks = loadTasks()
    val filtered = tasks.toMap.filter((id, task) => task.tags.contains(tag))
    Tasks(filtered)
  ...
  def tags: Tags =
    val tasks = loadTasks()
    val tags = tasks.toMap.flatMap((id,task) => task.tags).toList.distinct
    Tags(tags)
